...
Case-study galleries are rendered by `site/src/components/CaseStudyGallery.astro` from each page's typed `gallery` data. `setupCaseGalleries()` creates a client-side duplicate sequence for a seamless 10px/s loop, pauses while the pointer or keyboard focus is inside the gallery, and resumes only when it is visible and the tab is active. Keep the gallery static under `prefers-reduced-motion`; the native horizontal track remains manually scrollable. Preserve the borderless, title-free presentation and keep cloned links out of the focus order. Update the gallery interaction test whenever this behavior changes.
